...
INLINE void st8at (uint32_t x, uint8_t v) {
	static uint32_t x_cached[WITH_SMP] = {[0 ... WITH_SMP - 1] = 0};
	static unsigned long x_translated_cached[WITH_SMP] = {[0 ... WITH_SMP - 1] = 0};
...
INLINE void st32at (uint32_t x, uint32_t v) {
	if (x&0b11) {
		if (scpustate->curctx)
			...
			sim_io_eprintf (sd,
				"pu32-sim: core%u: %u bytes write to unaligned address 0x%x at 0x%x\n",
				PU32_SIM_CPU(scpu)->coreid, (int)sizeof(uint32_t), x, ip);
			dumpregs(scpu);
			sim_engine_halt (
				sd, scpu, scpu, ip,
				sim_stopped, SIM_SIGBUS);
	...
	if (x&0b1) {
		if (scpustate->curctx)
			...
			sim_io_eprintf (sd,
				...
				PU32_SIM_CPU(scpu)->coreid, (int)sizeof(uint16_t), x, ip);
			dumpregs(scpu);
			sim_engine_halt (
				sd, scpu, scpu, ip,
				sim_stopped, SIM_SIGBUS);
