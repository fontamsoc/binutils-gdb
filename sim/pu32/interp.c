...
#define STDIN_POLL_IDX (INTRSYNC_POLL_IDX+1)
#define TIMER_POLL_IDX (STDIN_POLL_IDX+1)
...
static void intrthread (unsigned coreid) {
	...
	while (1) {
		while (poll ((struct pollfd *)intrfds[coreid], POLL_NFDS, -1) == -1) {
			if (errno == EINTR)
				continue;
			perror("poll()");
...
int sim_read (
	...
	if (scpustate->curctx) {
		...
		if (x < PU32_KERNELSPACE_START || x >= scpustateregs[PU32_REG_KSL]) {
			uint32_t d2 = walk_page_table (x, 0 /*coreid*/);
			if (d2)
				x = ((d2 & PAGE_MASK) | (x & ~PAGE_MASK));
			else {
				#if defined(PU32_DEBUG)
...
SIM_DESC sim_open (
	...
	for (unsigned i = 0; i < PU32_CPUCNT; ++i) {
	...
	// ### PU32_CPUCNT must be kept 1
	// ### until the logic to start/stop
	// ### a CPU has been implemented.
	SIM_ASSERT(PU32_CPUCNT == 1);
	...
	if (sim_cpu_alloc_all (sd, PU32_CPUCNT, 0) != SIM_RC_OK) {
		free_state();
		return SIM_RC_FAIL;
	}
	...
	pu32state *states = mmap (0,
		ROUNDUPTOPOWEROFTWO(sizeof(pu32state)*PU32_CPUCNT, 0x1000),
	...
	for (unsigned long i = 0; i < PU32_CPUCNT; ++i) {
